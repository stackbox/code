...
vector <string> EllysDirectoryListing::getFiles(vector <string> files) {
	int i,swapPos = files.size()-1;
	for(i = 0; i < files.size(); i++)
	{
		if(files[files.size()-1] == ".." && files[files.size()-2] == "." || files[files.size()-1] == "." && files[files.size()-2] == "..") break;
		if(files[i] == ".." || files[i] == ".")
		{
			string tmp = files[swapPos];
			files[swapPos] = files[i];
			files[i] = tmp;
			swapPos--;
			i--;
		}
